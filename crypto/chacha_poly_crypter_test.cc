...
struct ChachaPolyTestVector {
  ...
  std::vector<uint8_t> ct;  // This contains the ciphertext and the tag.
  bool invalid_tag;
  bool invalid_nonce;
};
...
// Sample test vector from RFC 7539.
// <http://tools.ietf.org/html/rfc7539#section-2.8.2>
constexpr uint8_t kRFC7539TestVec1Data[] = {
...
constexpr uint8_t kRFC7539TestVec1AAD[] = {0x50, 0x51, 0x52, 0x53, 0xc0, 0xc1,
                                           0xc2, 0xc3, 0xc4, 0xc5, 0xc6, 0xc7};
constexpr uint8_t kRFC7539TestVec1Key[] = {
    0x80, 0x81, 0x82, 0x83, 0x84, 0x85, 0x86, 0x87, 0x88, 0x89, 0x8a,
    0x8b, 0x8c, 0x8d, 0x8e, 0x8f, 0x90, 0x91, 0x92, 0x93, 0x94, 0x95,
    0x96, 0x97, 0x98, 0x99, 0x9a, 0x9b, 0x9c, 0x9d, 0x9e, 0x9f};
constexpr uint8_t kRFC7539TestVec1Nonce[] = {
    0x07, 0x00, 0x00, 0x00, 0x40, 0x41, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47};
constexpr uint8_t kRFC7539TestVec1Ciphertext[] = {
...
TEST(ChachaPolyS2AAeadCrypterDeathTest, InvalidKeyLengthFailure) {
  std::vector<uint8_t> invalid_key = {};
  absl::variant<Status, std::unique_ptr<S2AAeadCrypter>> crypter_status =
      CreateChachaPolyAeadCrypter(invalid_key);
  ASSERT_EQ(crypter_status.index(), 0);
  EXPECT_EQ(absl::get<0>(crypter_status),
            Status(StatusCode::kInvalidArgument, "Key size is unsupported."));
}

TEST(ChachaPolyS2AAeadCrypterDeathTest, EncryptionFailure) {
  std::vector<uint8_t> RFC7539_test_vec1_nonce;
  RFC7539_test_vec1_nonce.assign(
      kRFC7539TestVec1Nonce,
      kRFC7539TestVec1Nonce + sizeof(kRFC7539TestVec1Nonce));

  std::vector<uint8_t> RFC7539_test_vec1_key;
  RFC7539_test_vec1_key.assign(
      kRFC7539TestVec1Key, kRFC7539TestVec1Key + sizeof(kRFC7539TestVec1Key));

  std::vector<uint8_t> RFC7539_test_vec1_plaintext;
  RFC7539_test_vec1_plaintext.assign(
      kRFC7539TestVec1Data,
      kRFC7539TestVec1Data + sizeof(kRFC7539TestVec1Data));

  absl::variant<Status, std::unique_ptr<S2AAeadCrypter>> crypter_status =
      CreateChachaPolyAeadCrypter(RFC7539_test_vec1_key);
...
// |testing::KilledBySignal| is not defined for those OS.
  ...
  EXPECT_EXIT(
      {
        S2AAeadCrypter::CrypterStatus encrypt_status =
            crypter->Encrypt(RFC7539_test_vec1_nonce, /*aad=*/{},
  ...
  ciphertext_and_tag.iov_base = &ciphertext_buf[0];
  ciphertext_and_tag.iov_len = ciphertext_buf.size();

  // Nonce length is invalid.
  std::vector<uint8_t> invalid_nonce = {'a', 'b', 'c'};
  S2AAeadCrypter::CrypterStatus encrypt_status = crypter->Encrypt(
      invalid_nonce, /*aad=*/{}, /*plaintext=*/{}, ciphertext_and_tag);
  EXPECT_THAT(
      encrypt_status.GetStatus(),
  ...
  encrypt_status = crypter->Encrypt(RFC7539_test_vec1_nonce, /*aad=*/{},
                                    invalid_plaintext_vec, ciphertext_and_tag);
  EXPECT_THAT(encrypt_status.GetStatus(),
              StatusIs(StatusCode::kInvalidArgument,
                       "non-zero plaintext length but plaintext is nullptr."));

  // Ciphertext and tag buffer is too small to hold the result of encryption.
  Iovec short_ciphertext_and_tag;
  std::vector<Iovec> plaintext_vec = {
      {RFC7539_test_vec1_plaintext.data(), RFC7539_test_vec1_plaintext.size()}};
  std::vector<uint8_t> short_ciphertext(crypter->MaxCiphertextAndTagLength(
      RFC7539_test_vec1_plaintext.size() - 1));
  short_ciphertext_and_tag.iov_base = &short_ciphertext[0];
  short_ciphertext_and_tag.iov_len = short_ciphertext.size();
  encrypt_status = crypter->Encrypt(RFC7539_test_vec1_nonce, /*aad=*/{},
                                    plaintext_vec, short_ciphertext_and_tag);
  EXPECT_THAT(
      encrypt_status.GetStatus(),
      StatusIs(StatusCode::kInvalidArgument,
               "|ciphertext_and_tag| is too small to hold the resulting "
               "ciphertext and tag."));

  // Additional data buffer is nullptr.
  std::vector<Iovec> invalid_aad_vec = {{/*iov_base=*/nullptr, /*iov_len=*/10}};
  ciphertext_buf.resize(
      crypter->MaxCiphertextAndTagLength(RFC7539_test_vec1_plaintext.size()));
  ciphertext_and_tag.iov_base = &ciphertext_buf[0];
  ciphertext_and_tag.iov_len = ciphertext_buf.size();
  encrypt_status = crypter->Encrypt(RFC7539_test_vec1_nonce, invalid_aad_vec,
...
TEST(ChachaPolyS2AAeadCrypterDeathTest, DecryptionFailure) {
  ...
  std::vector<uint8_t> RFC7539_test_vec1_aad;
  RFC7539_test_vec1_aad.assign(
      kRFC7539TestVec1AAD, kRFC7539TestVec1AAD + sizeof(kRFC7539TestVec1AAD));

  std::vector<uint8_t> RFC7539_test_vec1_ciphertext;
  RFC7539_test_vec1_ciphertext.assign(
      kRFC7539TestVec1Ciphertext,
      kRFC7539TestVec1Ciphertext + sizeof(kRFC7539TestVec1Ciphertext));
  absl::variant<Status, std::unique_ptr<S2AAeadCrypter>> crypter_status =
      CreateChachaPolyAeadCrypter(RFC7539_test_vec1_key);
  ASSERT_EQ(crypter_status.index(), 1);
  auto crypter = std::move(absl::get<1>(crypter_status));
...
// |testing::KilledBySignal| is not defined for those OS.
  ...
  EXPECT_EXIT(
      {
        S2AAeadCrypter::CrypterStatus decrypt_status =
            crypter->Decrypt(RFC7539_test_vec1_nonce, /*aad=*/{},
  ...
  Iovec plaintext;
  plaintext.iov_base = &plaintext_buf[0];
  plaintext.iov_len = plaintext_buf.size();
  ...
  short_ciphertext_and_tag_buf.assign(
      RFC7539_test_vec1_ciphertext.begin(),
      RFC7539_test_vec1_ciphertext.begin() + crypter->TagLength() - 1);
  short_ciphertext_and_tag_vec = {
      {&short_ciphertext_and_tag_buf[0], short_ciphertext_and_tag_buf.size()}};
  std::vector<Iovec> aad_vec = {
      {&RFC7539_test_vec1_nonce[0], RFC7539_test_vec1_nonce.size()}};
  decrypt_status = crypter->Decrypt(RFC7539_test_vec1_nonce, aad_vec,
                                    short_ciphertext_and_tag_vec, plaintext);
  EXPECT_THAT(decrypt_status.GetStatus(),
              StatusIs(StatusCode::kInvalidArgument,
                       "|ciphertext_and_tag| is too small to hold a tag."));

  // Plaintext buffer is too small to hold the result of decryption.
  Iovec short_plaintext;
  std::vector<Iovec> ciphertext_and_tag_vec = {
      {RFC7539_test_vec1_ciphertext.data(),
       RFC7539_test_vec1_ciphertext.size()}};
  std::vector<uint8_t> short_plaintext_buf(
      crypter->MaxPlaintextLength(RFC7539_test_vec1_ciphertext.size() - 1));
  ...
  // Additional data buffer is nullptr.
  std::vector<Iovec> invalid_aad_vec = {{/*iov_base=*/nullptr, /*iov_len=*/10}};
  plaintext_buf.resize(
      crypter->MaxPlaintextLength(RFC7539_test_vec1_ciphertext.size()));
  plaintext.iov_base = &plaintext_buf[0];
  plaintext.iov_len = plaintext_buf.size();
  decrypt_status = crypter->Decrypt(RFC7539_test_vec1_nonce, invalid_aad_vec,
                                    ciphertext_and_tag_vec, plaintext);
  EXPECT_THAT(decrypt_status.GetStatus(),
              StatusIs(StatusCode::kInvalidArgument,
                       "non-zero aad length but |aad| is nullptr."));
}

TEST(ChachaPolyS2AAeadCrypterTest, EncryptionDecryptionSuccess) {
  std::vector<uint8_t> RFC7539_test_vec1_nonce;
  RFC7539_test_vec1_nonce.assign(
      kRFC7539TestVec1Nonce,
      kRFC7539TestVec1Nonce + sizeof(kRFC7539TestVec1Nonce));
  ...
  for (int num_slices_plaintext = 0;
    ...
    std::vector<Iovec> plaintext_vec =
        S2AAeadSliceVector(RFC7539_test_vec1_plaintext, num_slices_plaintext);
    ...
    for (int num_slices_aad = 0; num_slices_aad < RFC7539_test_vec1_aad.size();
      ...
      std::vector<uint8_t> ciphertext_buf(crypter->MaxCiphertextAndTagLength(
          RFC7539_test_vec1_plaintext.size()));
      Iovec ciphertext_and_tag;
      ciphertext_and_tag.iov_base = &ciphertext_buf[0];
      ciphertext_and_tag.iov_len = ciphertext_buf.size();
      ...
      // Do encryption.
      S2AAeadCrypter::CrypterStatus encrypt_status = crypter->Encrypt(
          RFC7539_test_vec1_nonce, aad_vec, plaintext_vec, ciphertext_and_tag);
      EXPECT_EQ(encrypt_status.GetStatus(), Status());
      EXPECT_EQ(encrypt_status.GetBytesWritten(), ciphertext_and_tag.iov_len);
      EXPECT_EQ(memcmp(kRFC7539TestVec1Ciphertext, ciphertext_and_tag.iov_base,
                       sizeof(kRFC7539TestVec1Ciphertext)),
                0);
      ...
      for (int num_slices_ciphertext = 0;
        ...
        std::vector<Iovec> ciphertext_and_tag_vec = S2AAeadSliceVector(
            RFC7539_test_vec1_ciphertext, num_slices_ciphertext);
        ASSERT_EQ(ciphertext_and_tag_vec.size(), num_slices_ciphertext + 1);

        std::vector<uint8_t> plaintext_buf(
            crypter->MaxPlaintextLength(ciphertext_and_tag.iov_len));
        ...
        // Do decryption.
        S2AAeadCrypter::CrypterStatus decrypt_status =
            crypter->Decrypt(RFC7539_test_vec1_nonce, aad_vec,
                             ciphertext_and_tag_vec, plaintext);
        EXPECT_EQ(decrypt_status.GetStatus(), Status());
        EXPECT_EQ(decrypt_status.GetBytesWritten(), plaintext.iov_len);
        EXPECT_EQ(memcmp(kRFC7539TestVec1Data, plaintext.iov_base,
                         sizeof(kRFC7539TestVec1Data)),
                  0);

        S2AAeadSliceVectorCleanup(ciphertext_and_tag_vec);
      }
      S2AAeadSliceVectorCleanup(aad_vec);
    }
    S2AAeadSliceVectorCleanup(plaintext_vec);
  }
}
...
class ChaChaPolyS2AAeadCrypterTestVectors : public ::testing::Test {
 public:
  void DoEncryptDecrypt(const ChachaPolyTestVector& test_vector) {
    ...
    // Create Iovec vectors for aad and plaintext.
  ...
  void EncryptDecrypt(const ChachaPolyTestVector& test_vector) {
    // Init a valid crypter.
 ...
 private:
  void InitCrypter(const std::vector<uint8_t>& key) {
    absl::variant<Status, std::unique_ptr<S2AAeadCrypter>> crypter_status =
        CreateChachaPolyAeadCrypter(key);
    ASSERT_EQ(crypter_status.index(), 1);
...
TEST_F(ChaChaPolyS2AAeadCrypterTestVectors, RFC7539TestVectors) {
  // TODO(matthewstevenson88): Move test vectors to a separate test data
  // directory.
  ...
  const std::vector<ChachaPolyTestVector> chaCha20_rfc7539_test_vectors = {
      {0,
       {0x4c, 0x61, 0x64, 0x69, 0x65, 0x73, 0x20, 0x61, 0x6e, 0x64, 0x20, 0x47,
        0x65, 0x6e, 0x74, 0x6c, 0x65, 0x6d, 0x65, 0x6e, 0x20, 0x6f, 0x66, 0x20,
        0x74, 0x68, 0x65, 0x20, 0x63, 0x6c, 0x61, 0x73, 0x73, 0x20, 0x6f, 0x66,
      ...
      {1,
       ...
       false,
       false}};
  for (const auto& vector : chaCha20_rfc7539_test_vectors) {
    EncryptDecrypt(vector);
  }
}

TEST_F(ChaChaPolyS2AAeadCrypterTestVectors, WycheproofTestVectors) {
  // TODO(matthewstevenson88): Move test vectors to a separate test data
  // directory.

  // Testvectors from project wycheproof
  // <https://github.com/google/wycheproof>
  const ChachaPolyTestVector chaCha20_wycheproof_test_vectors[] = {
      // Comment: rfc7539
      {0,
       {0x4c, 0x61, 0x64, 0x69, 0x65, 0x73, 0x20, 0x61, 0x6e, 0x64, 0x20, 0x47,
        0x65, 0x6e, 0x74, 0x6c, 0x65, 0x6d, 0x65, 0x6e, 0x20, 0x6f, 0x66, 0x20,
  ...
  for (const auto& vector : chaCha20_wycheproof_test_vectors) {
    EncryptDecrypt(vector);
  }
}  // NOLINT
